...
export async function POST(req: NextRequest) {
  ...
  try {
    body = await req.json() as { emailConfirmacao?: string };
  } catch {
  ...
  try {
    ...
    logError("conta/excluir POST", err);
    return NextResponse.json({ error: "Falha ao processar exclusão de conta" }, { status: 500 });
  }
}
