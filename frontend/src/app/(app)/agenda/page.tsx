...
type SU = { role?: string; id?: string; moradaId?: string | null };
...
function AgendamentoFormDialog({
  ...
  function handleFormacaoChange(formacaoId: string) {
    const f = formacoes.find((x) => x.id === formacaoId);
    if (!f) return;
    setForm((prev) => ({ ...prev, formacaoId }));
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const dataFimISO = form.dataFim
        ? new Date(form.dataFim).toISOString()
        : dataInicioISO;

      const moradaIdFinal = isFC
        ? (userMoradaId ?? undefined)
      ...
      const payload: Agendamento = {
        ...
        nivelFormativo: (formacao?.nivelFormativo ?? "pre-discipulado") as NivelFormativo,
  ...
  const formacoesFiltradas = formacoes;
        ...
        <div className="grid gap-4 py-2">
          <div className="grid gap-1.5">
            ...
            <Select
              ...
              onValueChange={(v) => v && handleFormacaoChange(v)}
              items={Object.fromEntries(formacoesFiltradas.map((f) => [f.id, f.tema]))}
            >
              <SelectTrigger>
              ...
              <SelectContent>
                {formacoesFiltradas.map((f) => (
                  <SelectItem key={f.id} value={f.id}>
                    <span>{f.tema}</span>
                    {f.eixoNome && (
                      <span className="ml-2 text-xs text-muted-foreground">({f.eixoNome})</span>
                    )}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                placeholder="Salão, online, endereço..."
              />
            </div>
            <div className="grid gap-1.5">
                ...
                <div className="flex gap-1.5 shrink-0">
                  {ag.status === "agendada" && (
                    <>
                      <Button
                        ...
                        onClick={() => onStatusChange(ag.id, "confirmada")}
                      >
                        Confirmar
                      </Button>
                      <Button
                        variant="ghost" size="sm" className="h-7 text-xs text-red-600 hover:text-red-700 hover:bg-red-50"
                        onClick={() => onStatusChange(ag.id, "cancelada")}
                      >
                        Cancelar
                      </Button>
                    </>
                  ...
                  {ag.status === "confirmada" && (
                    <Button
                      ...
                      onClick={() => onStatusChange(ag.id, "realizada")}
                    >
...
export default function AgendaPage() {
  ...
  const role = user?.role ?? "formador_comunitario";
  const userId = user?.id ?? "";
  const userMoradaId = user?.moradaId ?? null;
  const isFC = role === "formador_comunitario";
  const canEdit = true; // all authenticated users can manage their agendamentos
  ...
  // FC vê apenas agendamentos da sua morada ou conduzidos por ele
  const meus = isFC
    ? agendamentos.filter((a) => a.moradaId === userMoradaId || a.formadorId === userId)
    : agendamentos;
  ...
  function handleStatusChange(id: string, newStatus: StatusFormacao) {
  ...
  function handleNovoSave(ag: Agendamento) {
    setAgendamentos((prev) => [...prev, ag]);
  }

  // Sorted by date for list view
  ...
  return (
    <div className="space-y-5 animate-in-fast">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        {STATUS_LIST.map((s) => {
          const count =
            s === "todos" ? meus.length : meus.filter((a) => a.status === s).length;
          return (
            <button
              key={s}
          ...
          <div className="flex flex-col items-center py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {statusFilter !== "todos"
                ...
                : "Clique em Agendar Formação para adicionar."}
            </p>
          </div>
        ) : (
          <>
