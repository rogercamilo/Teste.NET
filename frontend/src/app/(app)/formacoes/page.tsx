...
export default function FormacoesPage() {
  ...
  const moradaId = (session?.user as { moradaId?: string })?.moradaId;
  ...
  // Base set: restricted for formador_comunitario to their etapa only
  const baseFormacoes = isFormadorComunitario && myNivel
    ? formacoes.filter((f) => f.nivelFormativo === myNivel)
    : formacoes;

  // Apply search + grade filters (nivelFilter is handled at section level)
  ...
  // Determine which niveis are visible for this user
  const niveisVisiveis: NivelFormativo[] = isFormadorComunitario && myNivel
    ? [myNivel]
    : NIVEIS_ORDEM;

  // Group filtered formações by nivelFormativo, respecting nivelFilter
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Grouped sections by etapa formativa */}
      {niveisComFormacoes.length > 0 && (
              ...
              return (
                <section key={nivel}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <Badge
                      variant="outline"
                      className={`text-xs ${NIVEL_CORES[nivel]}`}
                    >
