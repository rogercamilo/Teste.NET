...
| 1.7 | ✅ **Agendamento para vários grupos** — junção N:N `AgendamentoGrupo` retrocompatível (grupoFormacaoId sincronizado; junção = fonte da verdade p/ notificações/lembretes/e-mail/RSVP). Criação/edição aceitam `grupoFormacaoIds[]` (multi-seleção por chips na Agenda; guard do FC = só a própria morada); lembretes/e-mail pela UNIÃO dos grupos (FCs deduplicados); GET/portal/RSVP reconhecem o grupo do formando via junção. Migração aditiva + backfill | Precisão do alcance de convites/lembretes ↔ Agenda, Grupos, Push | M | `Plan` → `feature-dev` |
...
| 2.3 | ✅ **Micro-interações & polish mobile** (transições, skeletons, toasts). Skeletons de rota (`ui/skeletons.tsx` + 14 `loading.tsx`) com **shimmer**; `PageTransition` (fade-in consistente por navegação, key=pathname); toasts (sonner) já adotados; cards de lista já com hover/transition; animações respeitam `prefers-reduced-motion` | Sensação premium | P/M | `frontend-design` |
...
| 2.5 | ✅ **Tema do tenant nas páginas de token do portal** — em `/portal/ativar/[token]` e `/portal/recuperar/[token]` o nome/logo já vinha da org certa (via token), mas o **tema CSS** injetado pelo `portal/layout.tsx` caía no DEFAULT (o layout só resolve a org pelo cookie de sessão, ausente durante ativação/reset). Fix: componente server `PortalThemeStyle` renderiza um `<style>` `:root{}` com o `temaCor` já resolvido pelo token, **depois** do style default do layout no DOM — mesma especificidade, declaração posterior vence → cor do tenant aplicada no 1º paint, sem flash. Sem plumbing de header/contexto. Descoberto na verificação do fix de branding cross-tenant (`58bc8ba`) | Consistência visual no 1º acesso | P | `frontend-design` |
