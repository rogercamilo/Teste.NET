...
| 2.5 | ⬜ *(baixa prioridade)* **Tema do tenant nas páginas de token do portal** — em `/portal/ativar/[token]` e `/portal/recuperar/[token]` o nome/logo já é da org correta (via token), mas o **tema CSS** injetado pelo `portal/layout.tsx` cai no DEFAULT, pois o layout só resolve a org pelo cookie de sessão (ausente durante ativação/reset, usuário deslogado). Fix exigiria o layout enxergar o token da rota aninhada (ex.: resolver a org no próprio segmento e passar via header/contexto). Cosmético; não afeta o dashboard autenticado. Descoberto na verificação do fix de branding cross-tenant (`58bc8ba`) | Consistência visual no 1º acesso | P | `frontend-design` |
